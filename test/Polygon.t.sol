...
contract PolygonTest is Test {
    ...
    // function testRevert_updateHubInflationRateTooEarly(uint64 time, uint256 rate) external {
    //     vm.assume(time < polygon.inflationRateModificationTimestamp() && rate < 1e3);
    //     vm.warp(time);
    ...
    // function testRevert_updateHubInflationRateInvalid(uint64 time, uint256 rate) external {
    //     vm.assume(time >= polygon.inflationRateModificationTimestamp() && rate >= 1e3);
    //     vm.warp(time);
    //     vm.startPrank(polygon.owner());
    //     vm.expectRevert(abi.encodeWithSelector(Polygon.Invalid.selector, "inflation rate must be less than previous"));
    //     polygon.updateHubInflation(rate);
    // }

    // function testRevert_updateTreasuryInflationRateTooEarly(uint64 time, uint256 rate) external {
    //     vm.assume(time < polygon.inflationRateModificationTimestamp() && rate < 1e3);
    //     vm.warp(time);
    //     vm.startPrank(polygon.owner());
    //     vm.expectRevert(abi.encodeWithSelector(Polygon.Invalid.selector, "inflation rate cannot be modified yet"));
    ...
    // function testRevert_updateTreasuryInflationRateInvalid(uint64 time, uint256 rate) external {
    //     vm.assume(time >= polygon.inflationRateModificationTimestamp() && rate >= 1e3);
    //     vm.warp(time);
    //     vm.startPrank(polygon.owner());
    //     vm.expectRevert(abi.encodeWithSelector(Polygon.Invalid.selector, "inflation rate must be less than previous"));
